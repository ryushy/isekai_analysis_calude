...
### キャラ入りの作り方（重要：静止画は一発生成・合成しない）

> **教訓**：完成した背景プレートに後からキャラだけ重ねる（image-to-image の合成）と、**背景とキャラのトーン（光・色・タッチ）がなじまず違和感**が出やすい。**静止画でキャラを入れる時は、背景＋キャラを最初から一緒に描く「一発生成」**にする。

- **静止画（観賞用ハイライト等）**：背景プレートと**同じシーン記述＋同じタッチ（hl_01基準）に、キャラの所作を足した一枚のプロンプト**で生成する（プレートを下絵に重ねない）。背景プレートは構図・トーンの**下調べ／参照**として使い、本番はキャラ込みで描き切る。
- **動画（[seedance](../seedance/SKILL.md)）**：こちらは別。背景プレートとキャラ正典を**別参照**で渡してエンジン側で合成する（動画は時間方向の制御が要るため）。
- **プレートの用途**：再利用基盤＋動画素材＋構図/トーンの確定。静止画キャラ版の「焼き込み下絵」には使わない。

### 直接 codex 運用時の納品物＝プロンプト

ユーザーが codex で直接生成する場合、このスキルの成果物は**プロンプト一式**（各カットの「背景プロンプト」＋「キャラ込みプロンプト」＋渡す参照の指定）。生成・レビューはユーザー側。例＝[ep1 highlights の 02_codex_prompts.md](../../../media/book1/ep1/highlights/02_codex_prompts.md)。
