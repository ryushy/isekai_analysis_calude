...
15. **【次の一手・著者決定待ち＝blocker】第8話の本文化**（B1-08）。**残る著者決定2点＝①引きの視点（ナギ＝罪悪感と決意／澪＝板挟みの孤独）②タイトル（露見／なかったことに／期限）**。決まり次第、本文化→4レビュー。または **第9話プロット**（B1-09「核へ」＝go dark本番・核へ潜る・削れに頼るか自力か・澪と協力）を5レビューで先行も可。
16. 任意：巻1の縦糸（削れの天秤／監視／澪の仲間化／喪失／道化の気配）の出し入れを第2〜巻末ブロックで配分（[book1_skeleton.md §2](../plot/book1_skeleton.md)）。
17. 任意：ストーリーボード（隠れ家establishing は確定済アセットあり／点vs面の共闘絵は第4話／鵜＝白い光の意匠／第7話＝点vs面の攻めの三速度＝緋色の装束が翻り矢が縦に貫く9コマ／第8話＝既視感のマッチカット・鵜の光が動線に灯る引き）。

> 進捗：第2ブロックは第5〜7話まで本文 v0.2 完成（鵜飼人物設計＋戻り標/シード機構C-036＋ep5-7プロット＋第5〜7話本文）。**第7話「管轄外」（道化のB1初再来＝気配のみ・C-037）まで本セッションで本文化＋4レビュー収束反映済**。次は第8話「露見」プロット（鵜飼が澪に直接迫る・第2ブロックの山）。
