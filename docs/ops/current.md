...
- ✅ **第3話プロット v0.1**：[ep3_plot.md](../plot/ep3_plot.md)「終わらない終着駅」＝**拠点獲得＋説明会**（再アクセス→当たりインスタンス→**タグ固定**→冷たい廃駅→温かい隠れ家）＋**初めてここで眠る＝削れの種（実害なし・予感のみ）**。固有現象＝終わらない終着駅（待ち続けて発てなかった人＝喪失の鏡）＝[canon_log C-029](../series/canon_log.md) 確定。澪は不在（共闘は第4話）。
- ✅ **第4話プロット v0.2**：[ep4_plot.md](../plot/ep4_plot.md)「眠っている間に」＝**特訓本番＋削れの代償（L1→L2・実害）＋澪共闘（点vs面の補完＝最初の仲間へ）**。第3話の"種"を実害で払う。
...
2. 続けて第4話本文化（[ep4_plot.md](../plot/ep4_plot.md)）＝削れの実害の中身（おばさんの口ぐせ等・親由来は伏せる）を確定。
