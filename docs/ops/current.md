...
10. **【次の一手】第6話の本文化**（B1-06）：v0.2 をもとに約5,500〜6,500字。または第7話プロット（管轄外＝道化の宿題）へ。
11. 任意：巻1の縦糸（削れの天秤／監視／澪の仲間化／喪失／道化の気配）の出し入れを第2〜巻末ブロックで配分（[book1_skeleton.md §2](../plot/book1_skeleton.md)）。
12. 任意：ストーリーボード（隠れ家establishing は確定済アセットあり／点vs面の共闘絵は第4話／鵜＝白い光の意匠）。
