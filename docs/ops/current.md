...
18. **【次の一手】第10話プロット**（B1-10「足跡の手前」＝**go darkの一歩手前で挫く＋鵜飼の決定的接触＝第3ブロックへの跳躍台**・第2ブロックのクライマックス前夜）。**プロットは5レビューで回す**。第2ブロック（B1-05〜10）の最終話。
19. 任意：巻1の縦糸（削れの天秤／監視／澪の仲間化／喪失／道化の気配）の出し入れを第2〜巻末ブロックで配分（[book1_skeleton.md §2](../plot/book1_skeleton.md)）。
20. 任意：ストーリーボード（隠れ家establishing は確定済アセットあり／点vs面の共闘絵は第4話／鵜＝白い光の意匠／第7話＝点vs面の攻めの三速度＝緋色の装束が翻り矢が縦に貫く9コマ／第8話＝既視感のマッチカット・鵜の光が動線に灯る引き／第9話＝核へ降りる漆黒・待っていた目の決め画）。
21. 任意：**字数計測の是正検討**＝check_manuscript.py が本文字数にヘッダ（`>`設計メモ）を含む件。物語本文のみを測るなら `>` 行も除外する改修を検討（過去回との整合に注意）。

> 進捗：第2ブロックは**第5〜9話まで本文 v0.2 完成**（鵜飼人物設計＋戻り標/シード機構C-036＋ep5-9プロット＋第5〜9話本文）。**本セッションで第7話本文化＋第8話プロット＋第8話本文化＋第9話プロット＋第9話本文化（go dark本番＝核へ）まで完了**（いずれも全レビュー収束反映・著者決定反映・main マージ済）。第2ブロックの山（露見）を越え、go dark本番（核の心臓に手は届くがシード一歩手前で引く）まで到達。残るは第10話「足跡の手前」（go darkを挫く＋鵜飼の決定的接触＝第3ブロックへの跳躍台）で第2ブロック完結。
