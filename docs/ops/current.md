...
19. **【次の一手・著者決定待ち＝blocker】第10話の本文化**（B1-10）。**残る著者決定＝タイトル（足跡の手前／逃げない／向き合う／二日）**（引きの視点はナギ視点で確定済）。決まり次第、本文化→4レビュー。or **第3ブロック（B1-11〜16＝出し抜く決着）の構想・第11話プロット**へ。
20. 任意：巻1の縦糸（削れの天秤／監視／澪の仲間化／喪失／道化の気配）の出し入れを第2〜巻末ブロックで配分（[book1_skeleton.md §2](../plot/book1_skeleton.md)）。
21. 任意：ストーリーボード（隠れ家establishing は確定済アセットあり／点vs面の共闘絵は第4話／鵜＝白い光の意匠／第7話＝点vs面の攻めの三速度／第8話＝既視感のマッチカット／第9話＝核へ降りる漆黒・待っていた目／第10話＝戻り標に手がかかる・整然と同期した白い目の面・ナギの背中→正面の振り返り）。
22. 任意：**字数計測の是正検討**＝check_manuscript.py が本文字数にヘッダ（`>`設計メモ）を含む件。物語本文のみを測るなら `>` 行も除外する改修を検討（過去回との整合に注意）。

> 進捗：**第2ブロック（B1-05〜10）はプロット全話完成・本文は第5〜9話まで v0.2 完成**（鵜飼人物設計＋戻り標/シード機構C-036＋ep5-10プロット＋第5〜9話本文）。**本セッションで第7話本文化＋第8話プロット＋第8話本文化＋第9話プロット＋第9話本文化＋第10話プロットまで完了**（7ユニット・全レビュー収束反映・著者決定反映・main マージ済）。go dark本番（核へ）→第10話で「go darkを挫く＋鵜飼の決定的接触＋澪の不可逆の第一手＋ナギの転回＝逃げない」で第2ブロックの設計が完結。残るは第10話の本文化（著者決定＝タイトルのみ）→第3ブロック（B1-11〜16＝出し抜く決着）の構想。
