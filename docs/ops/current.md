...
5. **【次の一手】第5話の本文化**（B1-05）：v0.2 をもとに約5,500〜6,500字。または著者レビューで顔出しの深さ（第5話で通告まで出すか）を確定してから。
6. その先：第2ブロック B1-06〜10 の話数配分（鵜飼の圧の段階・澪の露見の進度）。
7. 任意：巻1の縦糸（削れの天秤／監視／澪の仲間化／喪失／道化の気配）の出し入れを第2〜巻末ブロックで配分（[book1_skeleton.md §2](../plot/book1_skeleton.md)）。
8. 任意：ストーリーボード（隠れ家establishing は確定済アセットあり／点vs面の共闘絵は第4話）。
