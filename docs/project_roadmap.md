...
3. **機械検査＋セルフチェック**：`python scripts/check_manuscript.py <原稿> --min 5500 --max 6500` で AI 執筆の癖（git語露出・地の文太字・難語・専門語・英字混入・呼称ゆれ 等）を検出し NG ゼロに（[guides/ai_writing_pitfalls.md](guides/ai_writing_pitfalls.md)）。さらに**本文セルフチェック（§2.5）**＝比喩の伝達・心理と描写の一致・同語の多義・移動の自然さ・密度の四要素を目視。
